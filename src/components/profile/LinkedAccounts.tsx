...
const PROVIDERS: { id: OAuthProvider; label: string; icon: React.ReactNode; color: string }[] = [
  {
    id: "google",
    label: "Google",
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              {provider.icon}
              <span className="text-sm font-medium text-gray-200">{provider.label}</span>
            </div>
            {isLinked ? (
              <div className="flex items-center gap-2 text-green-400">
                ...
                <span className="text-xs font-medium">{dict.profile.linked}</span>
              </div>
            ) : (
              <button
                type="button"
