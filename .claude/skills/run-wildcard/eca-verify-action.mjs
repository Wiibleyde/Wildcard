...
const seed = await fetch(`${SB}/rest/v1/eca_games`, {
    method: "POST",
    headers: {
        apikey: SERVICE,
        Authorization: `Bearer ${SERVICE}`,
...
async function jpost(path, data) {
    ...
    if (!r.ok())
        throw new Error(`${path} -> ${r.status()} ${JSON.stringify(b)}`);
    return b;
}
...
const move =
    ...
    legal.find((a) => a.type === "drawCard") ??
    legal[0];
const played = await jpost(`/api/games/${gameId}/actions`, {
