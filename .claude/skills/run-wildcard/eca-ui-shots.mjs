...
async function auth() {
    const EMAIL = "ui-test@wildcard.local",
        PASSWORD = "ui-test-password-1234";
...
await fetch(`${SB}/rest/v1/eca_games`, {
    method: "POST",
    headers: {
        apikey: SERVICE,
        Authorization: `Bearer ${SERVICE}`,
...
async function shot(route, name, w, h) {
    ...
    page.on("pageerror", (e) =>
        errors.push(`${name} PAGEERROR ${e.message.slice(0, 160)}`),
    );
...
const room = await (
    await api.post(`${BASE}/api/rooms`, {
        data: {
            moduleId: `eca:${(await (await api.get(`${BASE}/api/studio/games`)).json()).games?.[0]?.id ?? ""}`,
        },
    })
)
    .json()
...
if (room?.code) {
    await api.post(`${BASE}/api/rooms/${room.code}/bots`, {
        data: { count: 2 },
    });
    const started = await (
        await api.post(`${BASE}/api/rooms/${room.code}/start`)
    ).json();
    if (started?.gameId) {
        report.push(
            await shot(`/fr/game/${started.gameId}`, "eca-game-375", 375, 820),
        );
        report.push(
            await shot(
                `/fr/game/${started.gameId}`,
                "eca-game-2560",
                2560,
                1080,
            ),
...
console.log(
    JSON.stringify(
        { report, consoleErrors: [...new Set(errors)].slice(0, 10) },
        null,
        1,
    ),
);
