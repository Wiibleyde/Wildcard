...
# Requires: supabase CLI  (bun i -g supabase  or  brew install supabase/tap/supabase)
# Usage: ./scripts/db-types.sh

# shellcheck source=_common.sh
source "$(dirname "$0")/_common.sh"

command -v supabase &>/dev/null || die "supabase CLI not found. Install: bun i -g supabase"
